...
def Track(tilt, pan, center, target = Point(100, 240), threshold = Point(16, 24), delta = Point(20, 20)):
    global position
    position = [100 , 220 ]
    if (center.x < target.x + threshold.x):
        position[0]=position[0]-delta.x
        print("xup = ", position)
        pwm.set_pwm(1, 0, position[0])
    ...
    if (center.y > target.y + threshold.y):
        position[1]=position[1]+delta.x
        print("xlr = ", position)
        pwm.set_pwm(0, 0, position[1])
...
if __name__ == "__main__":
    ...
    pan= pwm.set_pwm(0, 0, 90)
    #print (pan)
    tilt=pwm.set_pwm(1,0, 90)
    ...
    try:
      while 1:
        ...
        for (x, y, w, h) in faces:
           ...
           Track(tilt, pan, Point((x + x + w)/2.0, (y + y + h)/2.0))
           break
       # for (x, y, w, h) in faces:
